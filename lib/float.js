...
(function(global){
  ...
  // cal is the activity's own parsed calendar ({workingDays, exceptions})
  // or undefined - passed straight through to workdaysBetweenFn, which
  // falls back to Mon-Fri when it's undefined. Optional, so a caller that
  // doesn't pass a fourth argument here gets exactly today's behaviour.
  function computeTotalFloat(earlyFinish, lateFinish, workdaysBetweenFn, cal){
    return workdaysBetweenFn(earlyFinish, lateFinish, cal);
  }
